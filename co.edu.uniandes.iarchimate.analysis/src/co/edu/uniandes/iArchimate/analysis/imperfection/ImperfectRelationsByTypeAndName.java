...
public class ImperfectRelationsByTypeAndName extends AnalysisMethodImperfection {
	...
	public ImperfectRelationsByTypeAndName(){
		super();
	}
	...
	public void performAnalysisMethod(){
		try {
			...
			this.analysisMethodReport = new AnalysisMethodReport(new String[]{"Element","Relation","Total","Imperfect","Percentage"});			
			double p;
			for(String elementType : this.elementTypes){
				...
				if(elementHelper!=null){
					for(String relationTypeName : elementHelper.getRelationTypeNames()){
						...
						this.registry[3]=String.valueOf(relationHelper.getImperfectRelations());
						p = ((double)relationHelper.getImperfectRelations()/(double)relationHelper.getTotalRelations())*100;
						p = Math.round(p*100);
						p/=100;
